...
Development is active on the `develop` branch, and `atride.in` is live on Vercel. Phase 1 provides a populated marketplace, city filters, path-based public and private Guild pages, ride detail pages, SEO routes, and a health endpoint. Phase 2 includes Google OpenID Connect and email OTP sign-in, secure opaque sessions, logout, first-login onboarding, private participant profiles, optional one-time Firebase phone verification, a bike-first vehicle garage, and protected platform/Guild role boundaries. Phase 3 adds platform-admin Guild onboarding/approval, first-owner assignment, editable Guild identity and visibility, signed Cloudinary uploads, account-bound staff invitations, delegated role and membership management, operating cities and official links, presentation preferences, and tenant-scoped audit history. Phase 4 and 4B add the canonical multi-origin Ride Studio, vehicle-aware requirements, itinerary/stay/package/policies, origin-specific crew, media, controlled publication states, assigned-staff landing cards, approved-origin public widgets, PII-filtered WhatsApp-ready exports, and an optional review-before-apply Gemini Ride Assistant with an external fallback. Discovery pages read through tenant-scoped Prisma repositories backed by Neon PostgreSQL/PostGIS, with migrations, database constraints, and repeatable demonstration seeds in place.

Amazon SES domain authentication and sandbox access are configured; production-access approval is pending. Email OTP delivery supports a local mock and an SES v2 HTTPS adapter with narrowly scoped credentials. Firebase is scoped to one-time ownership verification of a saved operational phone. Cloudinary handles media, while the optional Gemini integration is server-only, usage-limited, and disabled by configuration when not wanted. The remaining Phase 0 foundation work is CI and operational hardening. Ride/service SMS, maps, Redis workers, and payments remain deliberately deferred behind development flows.
