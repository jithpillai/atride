...
- Phase 5 is implemented and user-tested end to end. It includes booking parties, room/occupancy pricing, bounded waitlists, assisted UPI and offline payment evidence, finance review, separate advance/balance obligations, participant manifests/exports, staff cancellation, whole-ride postponement/cancellation, refund reconciliation, and idempotent expiry/waitlist promotion.
...
Implementation status: **complete and user-tested**. Reservation, multi-person party booking, party-aware capacity and pricing, room-choice pricing/inventory, waitlist, consent snapshot, separate advance/balance obligations, offline UPI/bank/cash selection, Guild-level UPI configuration, immutable recipient snapshots, exact-amount UPI intents/local QR, transaction-reference capture, private proof upload, finance confirmation/rejection, durable payment email outbox, audit trail, personalized upcoming-rides integration, revocable member-only newcomer presentation, an authorized participant manifest, an audited Excel-compatible report, a privacy-limited WhatsApp roster, audited staff booking cancellation, whole-ride postponement/cancellation, Guild refund reconciliation, and the idempotent reservation-expiry/waitlist-promotion processor are implemented. Participant-requested transfer is deliberately excluded: authorized Guild staff cancel the old booking, while the replacement participant submits a fresh booking and accepts the current waiver and commercial terms. Production scheduler wiring is an operational deployment concern tracked with Phase 6/8 worker infrastructure rather than a missing booking capability.
