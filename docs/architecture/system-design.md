...
- `ride_disruptions` stores append-only postponement/cancellation decisions with reason, actor, proposed update, active/resolved state, and timestamps. A resolved postponement remains historical; cancellation is terminal.
- `booking_refunds` stores one aggregate cancellation reconciliation per booking: confirmed amount, submitted-but-unconfirmed evidence, cumulative refunded amount, status, reference, finance note, actor, and review time. It records a Guild-performed refund and never represents an @Ride settlement.

`media_assets` stores payment evidence using private authenticated Cloudinary delivery. `audit_logs` records reservation, disruption, and finance decisions. Separate general status-history, capacity-reservation, and waiver tables remain targets for later increments where their independent lifecycles are required.
...
Whole-ride disruption uses a separate locked transaction. Postponement keeps bookings unchanged but changes the ride to `POSTPONED`, which blocks reservations, cash/UPI prompts, upload signatures, and final proof attachment. Republish/draft transitions resolve the active postponement. Cancellation resolves older active disruptions, creates a terminal cancellation record, cancels every active booking without deletion, clears the derived booked-slot counter, and deliberately performs no waitlist promotion. Confirmed payments become `PENDING` refund records; submitted-but-unconfirmed evidence becomes `REVIEW_REQUIRED`. Participant outbox rows are committed with the disruption, and best-effort delivery runs only after commit.
