...
export function RideBookingForm({
  ...
  return <form id="booking" action={action} className="relative mt-6 grid gap-5 scroll-mt-28">
    ...
    {!soldOut && <label className="text-sm font-bold">Offline payment method<select name="paymentMethod" className="field bg-[#101419]"><option value="UPI">UPI</option><option value="BANK_TRANSFER">Bank transfer</option><option value="CASH">Cash — organizer confirmation required</option></select></label>}
    {soldOut && <><input type="hidden" name="paymentMethod" value="UPI" /><label className="flex cursor-pointer gap-3 rounded-2xl border border-amber-400/20 bg-amber-400/[.04] p-4 text-sm"><input required type="checkbox" name="joinWaitlistWhenFull" className="mt-1 accent-orange-500" /><span><strong>Join the waitlist</strong><span className="mt-1 block text-xs text-zinc-500">This does not reserve a slot or request payment. The Guild can contact you if capacity becomes available.</span></span></label></>}
