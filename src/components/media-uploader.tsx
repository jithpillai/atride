  ...
  async function upload() {
    ...
    try {
      for (const file of files) {
        const signResponse = await fetch("/api/media/sign", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ purpose, communitySlug, rideId }) });
        ...
        const completeResponse = await fetch("/api/media/complete", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ purpose, communitySlug, rideId, publicId: uploaded.public_id }) });
  ...
  async function remove() {
    ...
    try {
      const response = await fetch("/api/media/remove", {
        ...
        body: JSON.stringify({ assetId: currentAsset.id, communitySlug, rideId }),
      });
