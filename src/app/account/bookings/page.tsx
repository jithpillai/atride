...
export default async function BookingHistoryPage() {
  ...
  const bookings = await db.rideBooking.findMany({
    ...
    include: {
      ...
      ride: { select: { slug: true, title: true, destination: true, startsAt: true, endsAt: true, status: true } },
  ...
  const ordered = bookings
    .map((booking) => ({ booking, summary: summarizeBookingPayments(booking) }))
    .sort((left, right) => {
      const leftNeedsAction = Boolean(left.summary.activePayment && ["PENDING", "REJECTED"].includes(left.summary.activePayment.status));
      const rightNeedsAction = Boolean(right.summary.activePayment && ["PENDING", "REJECTED"].includes(right.summary.activePayment.status));
  ...
  const actionCount = ordered.filter(({ summary }) => summary.activePayment && ["PENDING", "REJECTED"].includes(summary.activePayment.status)).length;
  ...
  return <section className="mx-auto min-h-[70vh] max-w-6xl px-5 py-16 lg:px-8">
    ...
    <div className="mt-8 grid gap-6">
      {ordered.length ? ordered.map(({ booking, summary }) => {
          ...
          {!inactive && booking.status !== "WAITLISTED" && <section className="border-t border-white/10 bg-black/15 p-6 sm:p-7">
            ...
            <div className="mt-5 grid gap-4">{booking.payments.map((payment) => {
              ...
              return <div key={payment.id} className={`rounded-2xl border p-5 ${active && summary.overdue ? "border-red-400/25 bg-red-400/[.035]" : active ? "border-orange-400/20 bg-orange-400/[.025]" : "border-white/10"}`}>
                ...
                {active && payment.method === "CASH" && payment.status === "PENDING" && <p className="mt-3 text-xs leading-5 text-zinc-400">Cash selected. Pay the Guild directly; authorized finance staff will mark it received.</p>}
                {active && payment.method === "UPI" && ["PENDING", "REJECTED"].includes(payment.status) && payment.payeeVpaSnapshot && payment.payeeNameSnapshot && <UpiPaymentPanel paymentId={payment.id} vpa={payment.payeeVpaSnapshot} payeeName={payment.payeeNameSnapshot} amountPaise={payment.amountPaise} rideTitle={booking.ride.title} participantName={session.user.displayName} instructions={payment.payeeInstructionsSnapshot} />}
