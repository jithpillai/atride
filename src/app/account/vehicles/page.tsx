    ...
    <section className="mx-auto min-h-[70vh] max-w-5xl px-5 py-14 lg:px-8">
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-2">
        {vehicles.map((vehicle) => (
          <article key={vehicle.id} className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/[.025] p-6">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              ...
              {!vehicle.isPrimary && <form action={makePrimaryVehicle}><input type="hidden" name="id" value={vehicle.id} /><FormPendingSubmit idleLabel="Make primary" pendingLabel="Updating…" overlayLabel="Updating vehicle…" className="rounded-full border border-white/15 px-4 py-2 text-xs font-bold" /></form>}
              <form action={deleteVehicle}><input type="hidden" name="id" value={vehicle.id} /><FormPendingSubmit idleLabel="Remove" pendingLabel="Removing…" overlayLabel="Removing vehicle…" className="rounded-full border border-red-400/20 px-4 py-2 text-xs font-bold text-red-300" /></form>
            </div>
          </article>
        ))}
      ...
      <form action={createVehicle} className="relative mt-10 overflow-hidden rounded-3xl border border-white/10 bg-white/[.025] p-6 sm:p-8">
        ...
        <FormPendingSubmit idleLabel="Add vehicle" pendingLabel="Adding…" overlayLabel="Adding vehicle…" className="mt-8 rounded-full bg-orange-500 px-7 py-3.5 text-sm font-black text-white transition hover:bg-orange-400" />
      </form>
    </section>
  );
}
