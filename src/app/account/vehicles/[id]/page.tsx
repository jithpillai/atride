...
export default async function EditVehiclePage({ params, searchParams }: Props) {
  ...
  return (
    <section className="mx-auto min-h-[70vh] max-w-3xl px-5 py-14 lg:px-8">
      ...
      <form action={updateVehicle} className="relative mt-8 overflow-hidden rounded-3xl border border-white/10 bg-white/[.025] p-6 sm:p-8">
        <input type="hidden" name="id" value={vehicle.id} />
        <VehicleFields values={vehicle} />
        <FormPendingSubmit idleLabel="Save vehicle" pendingLabel="Saving…" overlayLabel="Saving vehicle…" className="mt-8 rounded-full bg-orange-500 px-7 py-3.5 text-sm font-black text-white transition hover:bg-orange-400" />
      </form>
    </section>
  );
}
