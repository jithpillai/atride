...
export default async function RidePage({ params, searchParams }: Props) {
  ...
  return <>
    ...
    <aside id="booking" className="h-fit min-w-0 scroll-mt-28 rounded-3xl border border-orange-500/25 bg-[#15191f] p-6 lg:sticky lg:top-28 lg:max-h-[calc(100vh-8rem)] lg:overflow-y-auto xl:p-7"><p className="text-sm text-zinc-500">Ride fee</p><p className="mt-1 text-3xl font-black">{formatMoney(ride.pricePaise / 100)}</p>{ride.confirmationDepositPaise > 0 && <p className="mt-2 text-xs text-zinc-500">₹{(ride.confirmationDepositPaise / 100).toLocaleString("en-IN")} confirmation deposit</p>}
