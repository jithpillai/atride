...
export default async function RidePage({ params, searchParams }: Props) {
  ...
  const isAssignedCrew = Boolean(session && ride.staffAssignments.some((assignment) => assignment.userId === session.userId));
  const whatsappInviteUrl = activeBooking?.status === "CONFIRMED" || isAssignedCrew
    ? decryptWhatsAppInviteUrl(ride.whatsappInviteEncrypted, ride.id)
    : null;
  const canOpenWhatsApp = Boolean(whatsappInviteUrl);
  ...
  const disruption = ride.disruptions[0] ?? null;
  const paymentActionsOpen = ride.status === "PUBLISHED" || ride.status === "CLOSED";
  ...
  return <>
    ...
    <section className="mx-auto grid max-w-7xl gap-10 px-5 py-16 lg:grid-cols-[minmax(0,1fr)_24rem] lg:px-8 xl:grid-cols-[minmax(0,1fr)_26rem]"><main className="min-w-0">
      ...
      {canOpenWhatsApp && <section className="mb-12 rounded-3xl border border-emerald-400/20 bg-emerald-400/[.025] p-6 sm:p-7"><p className="eyebrow">Optional participant chat</p><h2 className="mt-3 text-2xl font-black">Ride WhatsApp group</h2><p className="mt-3 max-w-3xl text-sm leading-7 text-zinc-400">This external group is manually managed by the ride organizers. Opening WhatsApp shares your account and phone details according to WhatsApp&apos;s own privacy settings. Essential ride instructions continue in @Ride even if you do not join.</p><a href={whatsappInviteUrl!} target="_blank" rel="noreferrer" className="mt-5 inline-flex cursor-pointer rounded-full bg-emerald-500 px-6 py-3 text-sm font-black text-black hover:bg-emerald-400">Open WhatsApp group</a></section>}
    ...
    <aside id="booking" className="h-fit min-w-0 scroll-mt-28 rounded-3xl border border-orange-500/25 bg-[#15191f] p-6 lg:sticky lg:top-28 lg:max-h-[calc(100vh-8rem)] lg:overflow-y-auto xl:p-7"><p className="text-sm text-zinc-500">Ride fee per person</p><p className="mt-1 text-3xl font-black">{formatMoney(ride.pricePaise / 100)}</p>{ride.confirmationDepositPaise > 0 && <p className="mt-2 text-xs text-zinc-500">₹{(ride.confirmationDepositPaise / 100).toLocaleString("en-IN")} confirmation deposit per person</p>}
      ...
      {booking?.status === "CANCELLED" ? <div className="mt-5 rounded-2xl border border-red-400/20 bg-red-400/[.035] p-5"><p className="text-xs font-bold uppercase tracking-widest text-red-300">Your cancelled booking</p><p className="mt-2 text-sm leading-6 text-zinc-300">The booking and its payment history have been preserved.</p>{booking.refund && <div className="mt-4 rounded-xl border border-white/10 p-4"><p className="text-xs text-zinc-500">Refund review</p><p className="mt-1 font-black text-amber-300">{booking.refund.status.replaceAll("_", " ")}</p><p className="mt-2 text-xs text-zinc-400">Refundable amount recorded: ₹{(booking.refund.confirmedAmountPaise / 100).toLocaleString("en-IN")} · Refunded: ₹{(booking.refund.refundedAmountPaise / 100).toLocaleString("en-IN")}</p></div>}</div>
      : activeBooking ? <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-5">
        ...
        {!paymentActionsOpen && <p className="mt-4 rounded-xl border border-amber-400/20 bg-amber-400/[.04] p-3 text-xs font-bold leading-5 text-amber-200">Booking and payment records are preserved, but payment actions are paused until the Guild republishes this ride.</p>}
        {activeBooking.status !== "WAITLISTED" && paymentSummary && <div className="mt-5 border-t border-white/10 pt-5">
          ...
          <div className="mt-4 grid gap-3">{activeBooking.payments.map((payment) => {
            ...
            return <div key={payment.id} className={`rounded-2xl border p-4 ${active && paymentSummary.overdue ? "border-red-400/25 bg-red-400/[.035]" : "border-white/10 bg-white/[.02]"}`}>
              ...
              {paymentActionsOpen && active && payment.method === "CASH" && payment.status === "PENDING" && <p className="mt-3 text-xs leading-5 text-zinc-400">Cash selected. Pay the Guild directly; only authorized finance staff can mark it received.</p>}
