...
export default async function RidePage({ params }: Props) {
  ...
  return <>
    <section className="relative overflow-hidden border-b border-white/10" style={{ background: ride.heroGradient }}>{ride.coverAsset && <ImageWithFallback src={cloudinaryImageUrl(ride.coverAsset)} fallbackSrc="/defaults/guild-hall-cover.png" alt="" fill sizes="100vw" priority className="object-cover opacity-55" />}<div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/35 to-transparent" /><div className="relative mx-auto max-w-7xl px-5 py-24 lg:px-8">
      ...
      <div className="mt-8 flex flex-wrap gap-3">{ride.staffAssignments.map((staff) => <span key={staff.id} className="rounded-full border border-white/20 bg-black/20 px-4 py-2 text-xs font-bold">{staff.role.replaceAll("_", " ")} · {staff.user.displayName}</span>)}</div>
    </div></section>
    ...
    <section className="mx-auto grid max-w-7xl gap-10 px-5 py-16 lg:grid-cols-[1fr_22rem] lg:px-8"><main className="min-w-0">
      ...
      <section className="mt-12"><p className="eyebrow">Starting groups</p><h2 className="mt-3 text-3xl font-black">Choose where your journey begins</h2><div className="mt-6 grid gap-4 sm:grid-cols-2">{ride.origins.map((origin) => <article key={origin.id} className="rounded-3xl border border-white/10 bg-white/[.025] p-6"><h3 className="text-xl font-black">{origin.city}</h3><p className="mt-2 text-sm text-zinc-400">{origin.meetingPoint}</p><p className="mt-4 text-sm font-bold text-orange-300">{when(origin.departureAt)}</p><p className="mt-2 text-xs text-zinc-600">{origin.capacity ? `${origin.capacity} planned riders` : "Flexible rider allocation"}{origin.mergePoint ? ` · merges at ${origin.mergePoint}` : ""}</p></article>)}</div></section>
    ...
    <aside className="h-fit rounded-3xl border border-orange-500/25 bg-[#15191f] p-7 lg:sticky lg:top-28"><p className="text-sm text-zinc-500">Ride fee</p><p className="mt-1 text-3xl font-black">{formatMoney(ride.pricePaise / 100)}</p>{ride.confirmationDepositPaise > 0 && <p className="mt-2 text-xs text-zinc-500">₹{(ride.confirmationDepositPaise / 100).toLocaleString("en-IN")} confirmation deposit</p>}
      ...
      <p className="mt-5 text-sm font-bold text-emerald-400">{slotsLeft} of {ride.totalSlots + ride.bufferSlots} slots available</p><Link href="/login" className="mt-5 block rounded-2xl bg-orange-500 px-5 py-3.5 text-center text-sm font-black text-white hover:bg-orange-400">Sign in to reserve</Link><p className="mt-4 text-center text-xs leading-5 text-zinc-600">Booking and payment actions arrive in Phase 5.</p>
    </aside></section>
  </>;
}
