...
export default async function GuildManagePage({ params, searchParams }: Props) {
  ...
  const [guild, state, pendingPayments, pendingRefunds] = await Promise.all([db.community.findUnique({
    where: { slug },
    include: {
      visibility: true,
    ...
    where: {
      booking: { community: { slug }, ride: { status: { in: ["PUBLISHED", "CLOSED"] } } },
      AND: [
    ...
    orderBy: { createdAt: "asc" },
    take: 50,
  }) : Promise.resolve([]), canFinance ? db.bookingRefund.findMany({
    where: { community: { slug }, status: { in: ["REVIEW_REQUIRED", "PENDING", "PARTIALLY_REFUNDED"] } },
    include: { booking: { include: { user: { select: { displayName: true } }, ride: { select: { title: true, slug: true } } } } },
    orderBy: { createdAt: "asc" },
    take: 50,
  }) : Promise.resolve([])]);
  if (!guild) notFound();
  ...
  return (
    <section className="mx-auto min-h-[70vh] max-w-6xl px-5 py-16 lg:px-8">
      ...
      {canFinance && activeSection === "finance" && <section id="refund-queue" className="mt-10 scroll-mt-28"><p className="eyebrow">Ride cancellations</p><h2 className="mt-3 text-2xl font-black">Refund reconciliation queue</h2><p className="mt-3 max-w-3xl text-sm leading-7 text-zinc-500">@Ride records what the Guild confirmed and what it reports as refunded; it does not move money. Submitted but unconfirmed payment proofs are marked for review so finance can establish the refundable amount before recording a refund.</p>{state.refundSaved && <p className="mt-4 text-sm font-bold text-emerald-300">Refund record updated.</p>}{state.refundError && <p className="mt-4 text-sm font-bold text-red-300">Check the refund amounts and provide a transaction reference when money was returned.</p>}<div className="mt-6 grid gap-4">{pendingRefunds.length ? pendingRefunds.map((refund) => <article key={refund.id} className="rounded-3xl border border-white/10 bg-white/[.025] p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-widest text-amber-300">{refund.status.replaceAll("_", " ")}</p><h3 className="mt-2 text-xl font-black">{refund.booking.user.displayName}</h3><p className="mt-1 text-sm text-zinc-400">{refund.booking.ride.title}</p><p className="mt-3 text-xs text-zinc-500">Confirmed payments: ₹{(refund.confirmedAmountPaise / 100).toLocaleString("en-IN")} · Submitted proofs awaiting reconciliation: ₹{(refund.submittedAmountPaise / 100).toLocaleString("en-IN")} · Already refunded: ₹{(refund.refundedAmountPaise / 100).toLocaleString("en-IN")}</p></div><Link href={`/rides/${refund.booking.ride.slug}`} className="w-fit rounded-full border border-white/15 px-4 py-2 text-xs font-bold">View ride</Link></div><form action={reviewBookingRefund} className="relative mt-5 grid gap-3 sm:grid-cols-2"><input type="hidden" name="guildSlug" value={guild.slug} /><input type="hidden" name="refundId" value={refund.id} /><label className="text-xs font-bold">Refundable amount (₹)<input required type="number" min="0" max={refund.booking.totalPricePaise / 100} step="0.01" name="confirmedAmount" defaultValue={refund.confirmedAmountPaise / 100} className="field mt-2 py-2.5 text-sm" /></label><label className="text-xs font-bold">Total refunded so far (₹)<input required type="number" min="0" step="0.01" name="refundedAmount" defaultValue={refund.refundedAmountPaise / 100} className="field mt-2 py-2.5 text-sm" /></label><label className="text-xs font-bold">Refund UTR / reference<input name="reference" maxLength={180} defaultValue={refund.reference ?? ""} placeholder="Required when amount refunded is above zero" className="field mt-2 py-2.5 text-sm" /></label><label className="text-xs font-bold">Finance note<input name="note" maxLength={1000} defaultValue={refund.note ?? ""} placeholder="Optional participant-facing reconciliation note" className="field mt-2 py-2.5 text-sm" /></label><div className="sm:col-span-2"><FormPendingSubmit idleLabel="Update refund record" pendingLabel="Updating…" overlayLabel="Updating refund reconciliation…" className="rounded-full bg-amber-400 px-5 py-2.5 text-xs font-black text-black" /></div></form></article>) : <p className="rounded-3xl border border-dashed border-white/10 p-8 text-center text-sm text-zinc-500">No cancellation refunds need attention.</p>}</div></section>}
