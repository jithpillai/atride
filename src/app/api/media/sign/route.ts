...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json() as { purpose?: unknown; communitySlug?: unknown };
    const purpose = typeof body.purpose === "string" ? body.purpose : "";
    if (!isSupportedMediaPurpose(purpose)) throw new AuthError("INVALID_MEDIA_PURPOSE", "This upload type is not supported.");
    const result = await createUploadSignature(session, purpose, typeof body.communitySlug === "string" ? body.communitySlug : undefined);
