...
export async function DELETE(request: Request) {
  try {
    ...
    const body = await request.json() as { assetId?: unknown; communitySlug?: unknown };
    if (typeof body.assetId !== "string") throw new AuthError("INVALID_MEDIA", "Select an image to remove.");
    await removeMediaAsset(session, body.assetId, typeof body.communitySlug === "string" ? body.communitySlug : undefined);
