...
describe("generateAnnouncementText", () => {
  it("uses one date for a same-day ride and the bike icon by default", () => {
    const content = generateAnnouncementText({
      ...
      pricePaise: 0, confirmationDepositPaise: 0, totalSlots: 10, waitlistCapacity: 0, bookedSlots: 0, status: "PUBLISHED", slug: "breakfast-run", vehicleType: "BIKE", distanceKm: 120, updatedAt: new Date(), community: { name: "Riders on a Break" },
  ...
  it("uses canonical data while excluding private stay and contact details", () => {
    const content = generateAnnouncementText({
      ...
      pricePaise: 389900, confirmationDepositPaise: 50000, totalSlots: 20, waitlistCapacity: 2, bookedSlots: 19, status: "PUBLISHED", slug: "agumbe-ride", vehicleType: "BIKE", distanceKm: 800, updatedAt: new Date(), community: { name: "Wild Gear Crew" },
    ...
    expect(content).toContain("1 slot currently available");
