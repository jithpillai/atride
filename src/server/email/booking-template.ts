...
export function renderBookingEventEmail(eventType: NotificationEventType, recipientName: string, payload: BookingEmailPayload) {
  const reserved = eventType === "BOOKING_RESERVED";
  const waitlisted = eventType === "BOOKING_WAITLISTED";
  ...
  const expiry = payload.reservationExpiresAt
    ...
    : null;
  const headline = reserved
    ? "Your ride reservation is held"
    : waitlisted
      ? "You joined the ride waitlist"
      : promoted
        ? "A ride slot is now held for you"
        : "Your ride reservation expired";
  const subject = reserved
    ? `${payload.rideTitle}: reservation received`
    : waitlisted
      ? `${payload.rideTitle}: waitlist joined`
      : promoted
    ? `${payload.rideTitle}: a waitlist slot is available`
    : `${payload.rideTitle}: reservation hold expired`;
  const party = payload.partySize && payload.partySize > 1 ? ` for all ${payload.partySize} participants` : "";
  const origin = payload.originName ? ` from ${payload.originName}` : "";
  const statusCopy = reserved
    ? `${payload.guildName} has placed a temporary reservation${party}${origin}. Complete the displayed payment${expiry ? ` before ${expiry}` : " within the displayed hold period"} to keep it.`
    : waitlisted
      ? `${payload.guildName} has added your complete booking party${origin} to the waitlist. You will be notified if enough capacity becomes available for everyone in the party.`
      : promoted
  ...
  const action = reserved || promoted ? "Complete reservation" : waitlisted ? "View waitlist status" : "View ride";
